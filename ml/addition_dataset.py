...
def _tuple_to_int(vals,d):
    #convert (d1,...,dk) to a unique integer, where d_i=0,1,...,d-1
    pow=[1]
    while len(pow)<len(vals):
        pow=[d*pow[0]]+pow
    return sum([a*b for a,b in zip(pow,vals)])
...
class GroupAddition(torch.utils.data.Dataset):
    def __init__(self,carry_table,depth,ids,debug_mode=False,digit_order='standard',input_format='onehot',interleaved=False,ans_at_end=False):
        ...
        self.ids=ids #a list of integers from 0 to d**(depth) indicating valid possibilites for g1 and g2
        ...
        self.len=100 if self.debug_mode else self.d**self.depth if self.ids is None else len(self.ids)
    ...
    def __getitem__(self, idx):
        while True:
            v1=np.random.choice(self.d,size=self.depth)
            if self.ids is None or _tuple_to_int(v1,self.d) in self.ids:
                break
        while True:
            v2 =np.random.choice(self.d, size=self.depth)
            if self.ids is None or _tuple_to_int(v2,self.d) in self.ids:
                break
        ...
        bases=[self.d**i for i in reversed(range(self.depth))]#used for integer representations
        ...
        if self.interleaved:
            inp=_interleave_lists(v1,v2)+[self.d]+[-1]*len(v1) if self.ans_at_end else _interleave_lists(v1,v2,[-1]*len(v1))
            ids=torch.Tensor([i for i,x in enumerate(inp) if x==-1]).long()
            if self.input_format=='onehot':
                X=torch.zeros((len(inp),self.d+(1 if self.ans_at_end else 0)))
            ...
            inp = np.array(list(v1) + [self.d]+list(v2))
            if self.input_format=='onehot':
                #convert to one-hots, with empty entries for filling in the answer
                X=torch.zeros((len(inp)+len(s),self.d+1))
                for i,j in enumerate(inp):
                    X[i,j]=1
            else:
                raise ValueError('')
