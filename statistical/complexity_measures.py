...
def compute_measures(args):
    ...
    measures = {}
    frac_zeros_vs_depth = {b: {c: [] for c in all_tables[b].keys()} for b in bases}
    num_digits_vs_depth = {b: {c: [] for c in all_tables[b].keys()} for b in bases}
    est_dim_box_vs_depth = {b: {c: [] for c in all_tables[b].keys()} for b in bases}

    # iterate through bases and c's
    for b in bases:
        for c in all_tables[b].keys():
            for depth in range(1, max_depth+1):
                table = fn.construct_product_table(all_tables[b][c], depth=depth)
